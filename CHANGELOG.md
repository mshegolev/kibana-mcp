...
- Кодовая база и тесты прогнаны через `ruff format` — гейт
  `ruff format --check src tests` в CI теперь проходит на чистом дереве.
  Изменения чисто типографские: переносы строк и склейка выражений,
  ни одной правки логики (`git diff v0.2.0..main --ignore-all-space`
  не содержит семантических хунков).

### Fixed

- `server.json` декларировал версию 0.1.1 в обоих полях, отставая от
  pyproject на целый релиз; теперь совпадает с версией пакета.
